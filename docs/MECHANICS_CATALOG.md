...
| FORM-01 | Normal flying formations | present | AREA-02, DIF-01 | sub: `flying_enemy_type_offset_tbl_normal`, `sub_2_fn_2__set_flying_enemies` | Fixtures preserve normal type, count, offsets, and order. The wave size + type-table offset are selected live — set-formation by the record offset, raise by the folded AI level, reset zeroes ([020](mechanics/020-normal-flying-formations.md)); foundation — selection is live/observable, spawning the wave is slice 8. |
