...
| SYS-02 | Shared entity lifecycle | present | SYS-01 | main: `add_obj_handler`, `handle_scroll_offscreen` | Every entity spawns, updates, takes one hit, explodes if applicable, and is removed without leaked state. 64-slot array + reset-clear built ([005](mechanics/005-entity-slot-lifecycle.md)); the index-order dispatch walk and live occupants (Toroids and enemy bullets, with position/velocity fields, hit state, and edge cull) are now live ([024](mechanics/024-toroid-vertical-slice.md), [026](mechanics/026-enemy-bullets-and-collision-death.md)); the operator playtest gates it. |
| SYS-03 | Collision groups and single-hit resolution | present | SYS-02 | main: `check_solvalou_hit`, `check_flying_enemies_shot`, `handle_bombed_obj_and_award_points` | Air, ground, player, bullet, and Bacura interactions stay distinct and cannot double-score. Single-hit path + five-group vocabulary built ([006](mechanics/006-collision-groups.md)); three groups are now live — player shots vs flying enemies ([025](mechanics/025-blaster-to-air-hit.md)) and both craft-hit groups (flying enemies and enemy bullets vs the craft, [026](mechanics/026-enemy-bullets-and-collision-death.md)); bombs-vs-ground (slice 9) and Bacura-vs-craft (slice 11) await their participants. |
| SYS-04 | Pseudo-random behavior | present | SYS-01 | main: `pseudo_random_gen`, `gen_rnd_dir` | Seeded fixtures repeat and dependent mechanics share one advancing random stream. Rule built and golden-verified ([004](mechanics/004-shared-random-stream.md)); the first live consumer is the Toroid spawn-column draw, whose per-tick draw order matches the model ([024](mechanics/024-toroid-vertical-slice.md), harness `rng-draw-order`). |
...
| PLY-02 | Player collision, explosion, life loss, and respawn | present | SYS-03, ECO-03 | main: `check_solvalou_hit` through `finish_solvalou_exploding` | Bullets, flying enemies, and Bacura cause one death and safe respawn or game over. The lives-driven respawn-vs-game-over decision, the fixed-point respawn with no invulnerability, and terrain-restart-on-death are built ([013](mechanics/013-death-decision-and-terrain-restart.md)); the real collision *trigger* is now live — a Toroid or an enemy bullet touching the craft raises the hit and runs the death path, retiring the D/G fixtures ([026](mechanics/026-enemy-bullets-and-collision-death.md)); Bacura-vs-craft (slice 11) and the death-audio cutoff (slice 20) remain. |
...
| WPN-02 | Blaster-to-air hit | present | WPN-01, SYS-03 | main: `check_flying_enemies_shot`, `check_shot_hit_flying_enemy`, `check_shot_hit_bacura` | A valid target reacts once; Bacura uses its distinct behavior. A player shot overlapping an active flying enemy resolves the hit once through the single-hit path, awards its value, and the enemy explodes and is freed ([025](mechanics/025-blaster-to-air-hit.md)); the Bacura bounce awaits Bacura (slice 11); the operator playtest gates it. |
...
| ID | Mechanic | Status | Depends on | Source labels | Acceptance outcome |
| --- | --- | --- | --- | --- | --- |
| ECO-01 | Object score, high score, and cap | present | SYS-03 | main: `add_to_score`, `update_high_score`, `set_score_to_9999990` | Recorded values update current/high score once and stop at the cap. The single `score` path — award, 9,999,990 cap, running high score, and the bonus hook — is built ([009](mechanics/009-scoring-path.md)); the real in-play producer is now live — destroying a Toroid awards its 30 points through this one path, retiring the debug S fixture ([025](mechanics/025-blaster-to-air-hit.md)); the operator playtest gates it. |
...
| AREA-02 | Area object scheduler | partial | AREA-01, SYS-02 | sub: `sub_fn_2__handle_objects`, `obj_fn_tbl`, `area_object_tbl_tbl_normal` | Normal events spawn in recorded order and position. The ordered schedule consume, a lossless representation of every area, and the `schedule fired` observable are built ([016](mechanics/016-area-scheduler.md), [017](mechanics/017-all-area-schedules.md)); the aerial-dispatch path is now live — formation records drive the live Toroid wave ([024](mechanics/024-toroid-vertical-slice.md)); the `add_object` ground-dispatch path (Barra/Logram) is slice 9, and the `add_object` flying branch (Torkan/Kapi/Terrazi) is slice 10. |
...
| DIF-03 | Per-family fire frequency | partial | DIF-01, SYS-04 | sub: `sub_2_fn_8__fire_freq_mask_derota` through `sub_2_fn_22__fire_freq_mask_andor_genesis` | Family fire fixtures follow recorded masks. The area schedule sets the eight per-family fire-permission masks + the ground-stop-firing row live from the record bytes ([022](mechanics/022-fire-permission-masks.md)); foundation — the masks are set/observable; slice 8's shooting Toroid fires exactly once *without* consuming any mask, so the fire-permission-gated firing (DIF-03.play, #60) is deliberately deferred to the mask-consuming families (slice 10). |
| FORM-01 | Normal flying formations | partial | AREA-02, DIF-01 | sub: `flying_enemy_type_offset_tbl_normal`, `sub_2_fn_2__set_flying_enemies` | Fixtures preserve normal type, count, offsets, and order. The wave size + type-table offset are selected live — set-formation by the record offset, raise by the folded AI level, reset zeroes ([020](mechanics/020-normal-flying-formations.md)); the formation now spawns live — the first `count` flying slots are continuously refilled from the type table until a reset row zeroes the count ([024](mechanics/024-toroid-vertical-slice.md)); the full adaptive-selection acceptance (FORM-01.play, #60) lands with the mask-consuming families (slice 10). |

## Flying enemies and projectiles

| ID | Mechanic | Status | Depends on | Source labels | Acceptance outcome |
| --- | --- | --- | --- | --- | --- |
| AIR-01 | Toroid movement, animation, firing variant | present | FORM-01, DIF-03 | main: `handle_0A_Toroid`, `handle_0B_Toroid_shoots`, `init_toroid` | A formation completes approach, swing, optional shot, exit, hit, and score. Toroids spawn from the live formation, aim-approach, swing laterally by the craft's side while flapping, the shooting variant fires one aimed bullet, and they cull or explode-and-score on a hit ([024](mechanics/024-toroid-vertical-slice.md), [026](mechanics/026-enemy-bullets-and-collision-death.md)); the operator playtest gates the look and feel. |
| AIR-02 | Torkan | built | FORM-01, DIF-03 | main: `handle_0F_Torkan`, `torkan_shoot`, `torkan_update_dir` | The three-phase coroutine — the aimed 2 px/frame approach with its `64–127`-frame shot delay, the single direct un-gated aimed shot at expiry (no mask), the in-place hover with the 28-frame seven-code animation, and the one-time 180° re-aim onto the fast 3 px/frame tier that breaks it **away** from the craft — plus the non-excluding random spawn draw and the six-frame roll render (with the shared explosion) are built ([029](mechanics/029-torkan-attack-and-retreat.md)); the operator playtest gates it. |
...
| AIR-05 | Kapi | partial | FORM-01, DIF-03 | main: `handle_10_Kapi`, `kapi_10_fire` | The silent aimed approach, the timer-triggered peel-away dive (lateral acceleration away from the craft's column with forward/scroll deceleration, latched once at dive entry), firing every dive tick through the shared fire-permission gate with no suppression, the non-excluding random spawn draw, and the 7-frame dive render (with the shared explosion) are built ([028](mechanics/028-kapi-peel-away-dive.md)); the operator playtest gates it. |
| AIR-06 | Terrazi | partial | FORM-01, DIF-03 | main: `handle_11_Terrazi`, `chk_timer_fire_bullet_reinit_timer` | The full attack/exit path and collisions complete. The aimed approach, the mask-gated periodic fire through the shared fire-permission gate, the glide-and-reverse near the craft, and the 7-frame roll render (with the shared explosion) are built ([027](mechanics/027-terrazi-and-fire-permission.md)); the operator playtest gates it. |
...
| AIR-12 | Standard and radiating bullets | partial | SYS-02, SYS-04 | main: `handle_06_Bullet`, `handle_07_Garu_Zakato_Bullet`, `init_new_bullet`, `init_radiating_bullet` | Bullets use recorded vectors, expire, and cause one hit. Standard aimed bullets are now live — a shooting Toroid fires one, it flies straight on its aimed vector, culls off any edge, and kills the craft on contact ([026](mechanics/026-enemy-bullets-and-collision-death.md)); the four-colour pulse and dedicated crops are deferred to a later art pass, and radiating bullets (Garu Zakato) are slice 11. |
