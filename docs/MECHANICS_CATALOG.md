...
| AIR-06 | Terrazi | partial | FORM-01, DIF-03 | main: `handle_11_Terrazi`, `chk_timer_fire_bullet_reinit_timer` | The full attack/exit path and collisions complete. The aimed approach, the mask-gated periodic fire through the shared fire-permission gate, and the glide-and-reverse near the craft are built and share the blaster hit window and explosion ([027](mechanics/027-terrazi-and-fire-permission.md)); the dedicated Terrazi sprite (a live slot is hidden until it is staged) and the operator playtest complete it. |
