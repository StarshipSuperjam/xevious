...
- **Game director** — the Stage is the sole writer of game state (title, ready, playing, player-dead,
  respawning, game-over, and later attract, high-score entry, player change) and owns every allowed
  transition.
...
- **Reference read at extraction time only.** The build and the game never touch the reference; the
  committed data files carry citations and hashes instead. Chosen over vendoring any part of the reference,
  which its license status forbids.
